  ...
  <v-layout class="fill-height">
    <v-navigation-drawer permanent>
      <v-list density="compact" nav>
        <v-list-item title="MSP Expert" subtitle="Work Area" />
    ...
    <v-main>
      <v-app-bar flat color="transparent">
        <v-spacer />
        <v-chip
          v-if="auth.isAdmin && auth.isActingAsExpert && auth.actingExpertLabel"
          class="mr-2"
          color="secondary"
          variant="tonal"
        >
          Acting as Expert: {{ auth.actingExpertLabel }}
        </v-chip>
        <v-chip v-if="auth.email" class="mr-2" color="primary" variant="tonal">
          {{ auth.email }}
        </v-chip>
        <v-btn
          v-if="auth.isAdmin && auth.isActingAsExpert"
          variant="text"
          @click="exitActing"
        >
          Back to Admin
...
const links = [
  ...
  { title: 'My Tickets', to: '/expert/tickets' },
